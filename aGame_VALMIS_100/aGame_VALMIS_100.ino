/* Created 2018 by: Jesper Ruuska, Juha Lallukka & Tapani Silvola
 * RISTINOLLA, MUSTIPELI & REAKTIOPELI - a project for Oulu University of Applied Sciences
 * 
 */
 

// define button target input values, got these via calibration
#define BUTTON_1 55
#define BUTTON_2 131
#define BUTTON_3 225
#define BUTTON_4 337
...
// bi-color led output pins. A = anode (long leg), C = cathode
...
volatile unsigned long ISR_waitTime = 0; // counter variable used by Wait & WaitInterruptable, incremented by Arduinos timer interrupt service routine
volatile unsigned ISR_delayTime = 0; // counter variable used by InputDelay, incremented by Arduinos timer interrupt service routine
...
void setup()
{
  ...
  OCR1A = 1999;// 1000 Hz interval, 8 prescaler.  (must be <65536 for timer1(16 bit))
  // turn on CTC mode
  TCCR1B |= (1 << WGM12);
...
void loop()
{
  ResetCheck(); // reset game variables if reset = true
  
  selectedGame = MenuAnim(); // game selection with animation, returns game selection as int
...
// reset function for game variables
void ResetCheck() {
  // reset game variables if reset = true
  if (reset) {
    // reset memory game:
    MG_answer = 0;
    MG_arraySize = 0;
...
// function for game selection screen, returns game selection (1, 2 or 3):
int MenuAnim() {
  ...
  int selection = 0;
  RG_ResetLeds();
  // loop this until button 1, 2 or 3 is read
  ...
  // save game selection and play transit animation
  ...
  Wait(600);
  RG_ResetLeds();
  return selection; // return selected game number
}
...
// function that resets the timer and leds
void RG_ResetLeds() {
  for(int i=1; i < 10; i++) {
...
{  
    ...
    WriteAllLedOutputs();       //note this function doesn't have build in wait(delay) in the end because it is used mainly during PLAYER INPUT sequence
}
...
//this can be also used to turn all LEDs off by setting colour to "OFF" and xtimes & time to 1
...
//BEGINING OF NEW ROUND
  MG_FlashAllLeds(GREEN, 3, 200); // beginning of a new round is indicated by flashing all LEDs green 3 times


//LED SEQUENCE
 for (int currentLed = 1; currentLed <= MG_arraySize; currentLed++) 
 { 
   MG_FlashOneLed(MG_sequence[currentLed], RED, 500); //the game flashes a corresponding LED red for each number previously added to the MG_sequence array 
   Wait(500);
 }
//led sequence END
...
//PLAYER INPUT
MG_FlashAllLeds(GREEN, 1, 50);                                      // beginning of the PLAYER INPUT sequence is indicated by a quick flash of green with all LEDs.
for (int currentLed = 1; currentLed <= MG_arraySize; currentLed++)  //during this sequence player must press buttons in same order that LEDs were previously flashed in the LED SEQUENCE
  {
    while (!reset) //waits a player input
    {
      ...
      //WRONG ANSWER
      if (MG_answer != MG_sequence[currentLed]) // if the button pressed isn't same as the current number in the sequence array, the WRONG ANSWER sequence begins
      {
          MG_FlashOneLed(MG_answer, RED, 50);  // this is first indicated by flashing the wrong LED red once and then by flashing all LEDs red 3 times
          Wait(200);
          MG_FlashAllLeds(RED, 3, 200);        

          
          Wait(1000);
          MG_FlashOneLed(MG_sequence[currentLed], GREEN, 200); //game shows the correct answer by flashing that LED green 3 times
          Wait(200);
          MG_FlashOneLed(MG_sequence[currentLed], GREEN, 200);
          Wait(200);
          MG_FlashOneLed(MG_sequence[currentLed], GREEN, 200);
          Wait(2000);
      ...
      //wrong answer END    
        ...
        //CORRECT ANSWER     
        MG_FlashOneLed(MG_sequence[currentLed], GREEN, 50);  //correct answer is indicated by flashing that LED green once      
        Wait(200);
        MG_answer = 0;
                  
        if (currentLed == MG_score +1) //score is increased by one if the player gets the whole sequence correct
        {
          MG_score++;
        }
      }
...
//returs back to the begining of MEMORY GAME -main loop, a new random number is generated and a new round begins.
/////////////////memory game END//////////////
...
void InputDelay() {
  noInterrupts();
  ISR_delayTime = 0; // this is being incremented by ISR
  interrupts();
  while(1) {
    if (ISR_delayTime >= 1) {
      break;
...
void Wait(unsigned long millisecondsToWait) {
  noInterrupts();
  ISR_waitTime = 0; // this is being incremented by ISR
  interrupts();
  // multiply by 2 to take into account the delay in GetButtonInput
  while(ISR_waitTime < millisecondsToWait*2 && !reset) {
    GetButtonInput();
...
void WaitInterruptable(unsigned long millisecondsToWait) {
  ...
  while(ISR_waitTime < millisecondsToWait*2 && !reset) {
    ...
    millisecondsToWait--;
    // interrupt wait loop on any button press
    if (activeButton != 0) {
      break;
    }
  }
